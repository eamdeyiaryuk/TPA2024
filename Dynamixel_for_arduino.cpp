...
void DX::rotate(int16_t spd) /* control motor rotation */
{
  if(motor_mode == 2)
  {
    ...
    packet[3] = 0x20;                 // address of moving speed 0x20
...
void DX::rotate(int16_t spd, int16_t pos) /* control joint mode rotation */
{
  if(motor_mode == 1)
  {
    ...
    pos = map(pos, pos_minRange, pos_maxRange, 0, 1023); //4095

  
    Packet[0] = id_motor;                  // motor id
    Packet[1] = 0x07;                      // lenght = instruction + 2 0x07
    Packet[2] = 0x03;                      // writing type
    Packet[3] = 0x1E;                      // address of goal position Packet[3] = 0x1E;
